...
/// Signature used by [PhotoDismiss] to indicate that it has been dismissed in
/// the given `direction`.
///
/// Used by [PhotoDismiss.onDismissed].
typedef DismissDirectionCallback = void Function(
    PhotoDismissDirection direction);

/// Signature used by [PhotoDismiss] to give the application an opportunity to
/// confirm or veto a dismiss gesture.
///
/// Used by [PhotoDismiss.confirmDismiss].
typedef ConfirmDismissCallback = Future<bool?> Function(
    PhotoDismissDirection direction);

/// Signature used by [PhotoDismiss] to indicate that the dismissible has been dragged.
///
/// Used by [PhotoDismiss.onUpdate].
typedef DismissUpdateCallback = void Function(DismissUpdateDetails details);

/// The direction in which a [PhotoDismiss] can be dismissed.
enum PhotoDismissDirection {
  /// The [PhotoDismiss] can be dismissed by dragging either up or down.
  vertical,

  /// The [PhotoDismiss] can be dismissed by dragging either left or right.
  horizontal,

  /// The [PhotoDismiss] can be dismissed by dragging in the reverse of the
  /// reading direction (e.g., from right to left in left-to-right languages).
  endToStart,

  /// The [PhotoDismiss] can be dismissed by dragging in the reading direction
  /// (e.g., from left to right in left-to-right languages).
  startToEnd,

  /// The [PhotoDismiss] can be dismissed by dragging up only.
  up,

  /// The [PhotoDismiss] can be dismissed by dragging down only.
  down,

  /// The [PhotoDismiss] cannot be dismissed by dragging.
  none
}

/// A widget that can be dismissed by dragging in the indicated [direction].
///
/// Dragging or flinging this widget in the [PhotoDismissDirection] causes the child
...
///
/// {@tool dartpad}
/// This sample shows how you can use the [PhotoDismiss] widget to
...
class PhotoDismiss extends StatefulWidget {
  /// Creates a widget that can be dismissed.
  ///
  /// The [key] argument is required because [PhotoDismiss]s are commonly used in
  ...
  const PhotoDismiss({
    ...
    this.onUpdate,
    this.onDismissed,
    this.direction = PhotoDismissDirection.horizontal,
    this.resizeDuration = const Duration(milliseconds: 300),
    this.dismissThresholds = const <PhotoDismissDirection, double>{},
  ...
  final PhotoDismissDirection direction;
  ...
  /// [PhotoDismissDirection] even if it would be allowed by the [direction]
  /// property.
  ///
  /// See also:
  ///
  ...
  ///    be dismissed.
  final Map<PhotoDismissDirection, double> dismissThresholds;
  ...
  @override
  State<PhotoDismiss> createState() => _PhotoDismissState();
}
...
/// See also:
///
///   * [PhotoDismiss.onUpdate], which receives this information.
class DismissUpdateDetails {
  ...
  /// that the [PhotoDismiss] is dragged across the threshold.
...
class _PhotoDismissState extends State<PhotoDismiss>
  ...
  @override
  bool get wantKeepAlive => _moveController.isAnimating;
  ...
  bool get _directionIsXAxis {
    return widget.direction == PhotoDismissDirection.horizontal ||
        widget.direction == PhotoDismissDirection.endToStart ||
        widget.direction == PhotoDismissDirection.startToEnd;
  }

  PhotoDismissDirection _extentToDirection(double extent) {
    if (extent == 0.0) {
      return PhotoDismissDirection.none;
    }
    if (_directionIsXAxis) {
      return switch (Directionality.of(context)) {
        TextDirection.rtl when extent < 0 => PhotoDismissDirection.startToEnd,
        TextDirection.ltr when extent > 0 => PhotoDismissDirection.startToEnd,
        TextDirection.rtl ||
        TextDirection.ltr =>
          PhotoDismissDirection.endToStart,
      };
    }
    return extent > 0 ? PhotoDismissDirection.down : PhotoDismissDirection.up;
  }

  PhotoDismissDirection get _dismissDirection =>
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    ...
    switch (widget.direction) {
      case PhotoDismissDirection.horizontal:
      case PhotoDismissDirection.vertical:
        _dragExtent += delta;

      case PhotoDismissDirection.up:
        if (_dragExtent + delta < 0) {
          _dragExtent += delta;
        }

      case PhotoDismissDirection.down:
        if (_dragExtent + delta > 0) {
          _dragExtent += delta;
        }

      case PhotoDismissDirection.endToStart:
      ...
      case PhotoDismissDirection.startToEnd:
        switch (Directionality.of(context)) {
          case TextDirection.rtl:
            if (_dragExtent + delta < 0) {
              _dragExtent += delta;
      ...
      case PhotoDismissDirection.none:
        _dragExtent = 0;
    }
  ...
  _FlingGestureKind _describeFlingGesture(Velocity velocity) {
    ...
    PhotoDismissDirection flingDirection;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.secondaryBackground != null) {
      final PhotoDismissDirection direction = _dismissDirection;
      if (direction == PhotoDismissDirection.endToStart ||
          direction == PhotoDismissDirection.up) {
    ...
    // cannot be dragged.
    if (widget.direction == PhotoDismissDirection.none) {
      return content;
    }
